  ...
  function apiModal(id, projectId) {
    ...
    showModal(
      ...
      commonFields(api, "API") +
        `${scopedProject ? `<div class="field"><label>Owning project</label><div class="readonly-field">${UI.esc(scopedProject.name)}</div><input type="hidden" name="owner" value="${UI.esc(scopedProject.id)}"></div>` : `<div class="field"><label>Owning project</label><select name="owner">${data.projects.map((p) => `<option value="${UI.esc(p.id)}" ${owner === p.id ? "selected" : ""}>${UI.esc(p.name)}</option>`).join("")}</select></div>`}<div class="field"><label>Lifecycle</label><select name="lifecycle">${["planned", "in-development", "operational", "on-hold", "archived"].map((x) => `<option value="${x}" ${life === x ? "selected" : ""}>${UI.title(x)}</option>`).join("")}</select></div><div class="field"><label>Shared base flow count</label><input type="number" min="0" name="base" value="${UI.esc(base)}"></div><div class="field full"><span class="group-label">Independent environment configuration</span><div class="env-grid">${["dev", "test", "prod"].map((e) => envFields(api, e)).join("")}</div></div></div>`,
      (fd) => {
        const month = fd.get("month"),
          newBase = +fd.get("base");
        if (
